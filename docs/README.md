# StepKit SDK Documentation

Official documentation site for StepKit SDK - bringing Inngest's reliability to the open source community.

## About StepKit SDK

StepKit SDK makes any TypeScript function durable, reliable, and observable. Built on Inngest's battle-tested reliability patterns, now open source and vendor-agnostic.

**Key Features:**
- 🔄 Automatic retries and error handling
- ⏸️ Pause and resume workflows without holding resources
- 🔍 Built-in observability for every step
- 🚀 Deploy anywhere - no vendor lock-in
- 💪 Type-safe with full TypeScript support
- 🌐 Framework agnostic (Next.js, Remix, Express, and more)

## Documentation Site

This repository contains the documentation site built with:
- [Fumadocs](https://fumadocs.dev) - Documentation framework
- [Tanstack Start](https://tanstack.com/start) - Full-stack React framework
- [Tailwind CSS](https://tailwindcss.com) - Styling

## Development

Run the development server:

```bash
npm run dev
# or
pnpm dev
# or
yarn dev
```

Open [http://localhost:3000](http://localhost:3000) to view the documentation site.

## Building for Production

```bash
npm run build
npm run start
```

## Contributing

We welcome contributions! Please see our contributing guidelines for more information.

## License

[MIT License](LICENSE) - see LICENSE file for details.

## Links

- [Documentation](https://step.run)
- [GitHub Repository](https://github.com/inngest/stepkit)
- [Inngest](https://inngest.com)
- [Community Discord](https://inngest.com/discord)
