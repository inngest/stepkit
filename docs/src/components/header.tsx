...
export function Header({ activeItem }: HeaderProps) {
  ...
  <nav class="w-full pt-6 uppercase tracking-normal font-mono">
    <div class="grid grid-cols-6 gap-4 items-stretch">
      <a href="/" class="h-full flex items-center justify-start space-x-2 border-t-4 border-white pt-2 hover:opacity-70">
        <span class="text-2xl font-bold leading-none">StepKit</span>
        <span class="text-[9px]">BY INNGEST</span>
      </a>

      <a href="/docs" class="h-full border-t-4 border-white pt-2 flex items-center justify-end text-right space-x-2 hover:opacity-70">
